    ...
    public readonly partial struct ContextName
    {
        ...
        public ValidationContext Validate(in ValidationContext validationContext, ValidationLevel level = ValidationLevel.Flag)
        {
            ...
            if (level > ValidationLevel.Basic)
            {
                ...
                result = result.PushSchemaLocation("contexts.json#/definitions/contextName");
            }
        ...
        public static partial class CorvusValidation
        {
                ...
                ValidationLevel level = ValidationLevel.Flag)
            {
                ...
                static bool StringValidator(ReadOnlySpan<char> input, in Corvus.Json.Validate.ValidationContextWrapper context, out ValidationContext result)
                {
                    ...
                    if (Pattern.IsMatch(input))
                    {
                        if (context.Level == ValidationLevel.Verbose)
                        {
                            result = result.WithResult(isValid: true, validationLocationReducedPathModifier: new JsonReference("pattern"), $"Validation pattern - {input.ToString()} matched '(^[a-z]|[A-Z0-9])[a-z]*$'");
                        }
                    }
                    else
                    {
                        ...
                        {
                            result = result.WithResult(isValid: false, validationLocationReducedPathModifier: new JsonReference("pattern"), $"Validation pattern - {input.ToString()} did not match '(^[a-z]|[A-Z0-9])[a-z]*$'");
                        }
                        else
                        {
                            result = result.WithResult(isValid: false, validationLocationReducedPathModifier: new JsonReference("pattern"), "Validation pattern - The value did not match '(^[a-z]|[A-Z0-9])[a-z]*$'");
                        }
                    }
            ...
            [GeneratedRegex("(^[a-z]|[A-Z0-9])[a-z]*$")]
            private static partial Regex CreatePattern();
#else
            private static Regex CreatePattern() => new("(^[a-z]|[A-Z0-9])[a-z]*$", RegexOptions.Compiled);
#endif
        }
    }
}
